...
cap_prod_fc_from_endo_model = CSV.read(joinpath(@__DIR__,"OutputData","Endogenous model direct neighbors","Cap_Prod_FC_endo_stor.csv"),DataFrame)
capstor_fc_from_endo_model = CSV.read(joinpath(@__DIR__,"OutputData","Endogenous model direct neighbors","CapStor_FC_endo_stor.csv"),DataFrame)
...
## Select day for which you would like to plot: 
#for jd = [1] # '160,165,342' or '1,2,364,365'
    if plot_figures == 1
        ## Call variables
    ...
    elseif plot_figures == 2
        ...
        if storage_included == "no"
            if cd_name == "FR00"
                #icd = [4,7,8,9,10,11] # Plot five main technologies together with nuclear for plots of generation and capacity mix of direct neighbouring country
            elseif cd_name == "FRNLUKDE"
        ...
        elseif storage_included == "yes"
            if cd_name == "FR00"
                #icd = [4,7,8,9,10,11] # Five main technologies together with nuclear for plots of generation FR
            elseif cd_name == "FRNLUKDE"
                ...
                CSV.write(joinpath(@__DIR__,"OutputData","Endogenous model direct neighbors","Cap_Prod_FC_endo_stor.csv"),  DataFrame("Technology" => Icd, "Capacity" => capacity_fc_from_endo_model, "Production" => production_fc_from_endo_model))
                CSV.write(joinpath(@__DIR__,"OutputData","Endogenous model direct neighbors","CapStor_FC_endo_stor.csv"),  DataFrame("Storage unit" => Istorcd, "Capacity" => capacity_storage_fc_from_endo_model))
                CSV.write(joinpath(@__DIR__,"OutputData","Endogenous model direct neighbors","Imp_Exp_FC_endo_stor.csv"),  DataFrame("Import" => total_import[fc_name], "Export" => total_export[fc_name]))
                CSV.write(joinpath(@__DIR__,"OutputData","Endogenous model direct neighbors","Cost_Comp_FC_endo_stor.csv"),  DataFrame("Cost Component" => COMP, "Value" => cost_fc_from_endo_model))
                CSV.write(joinpath(@__DIR__,"OutputData","Endogenous model direct neighbors","CompCost_endo_stor.csv"),  DataFrame("Solve time" => solve_time(m), "Variables" => num_variables(m), "Constraints" => num_constraints(m; count_variable_in_set_constraints = true), "Nonzeros" => MOI.get(m, Gurobi.ModelAttribute("NumNZs"))))
            end
        end
